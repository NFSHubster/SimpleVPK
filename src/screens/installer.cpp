...
static int downloader_main(unsigned int args, void* arg) {
	...
	for (;;) {
		...
		vita2d_pgf_draw_text(pgf,  20, 514, YELLOW, 1.0f, "based on vitaQuakeIII & VitaDB downloader by Rinnegatamante");
		...
		if (state > DOWNLOADING) {
			if (state >= FINISHED)
				vita2d_pgf_draw_textf(pgf, 20, 400, WHITE, 1.0f, "%s\nPress O to exit.", (install_state || dl_type) ? "Finished!" : (dl_isSimpleVPK ? "Press X to update." : "Press X to install. (May take several minutes)"));
...
void Installer::draw(SharedData &sharedData) {
	...
	dl_isSimpleVPK = sharedData.vpks[sharedData.cursorY]["titleid"].get<string>() == VITA_TITLEID;
